(() => {
  ...
  /* Credenciales de administrador - cambia antes de publicar */
  const ADMIN_USER = "acqua";
  const ADMIN_PASS = "AcquaNatacion!";
  ...
  const PLAN_CLASSES = { "1_practica": 1, "4": 4, "8": 8, "8_practicas": 8, "13": 13 };
  ...
  let isAuthenticated     = !!localStorage.getItem("acqua.session");
  ...
  async function cloudSave(snapshot) {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/app_state`, { method:"POST", headers:{ "Content-Type":"application/json", "apikey":SUPABASE_ANON_KEY, "Authorization":`Bearer ${SUPABASE_ANON_KEY}`, "Prefer":"resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ id:CLOUD_ROW_ID, data:snapshot, saved_at:new Date().toISOString() }) });
  ...
  async function cloudLoad() {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/app_state?id=eq.${encodeURIComponent(CLOUD_ROW_ID)}&select=data`, { cache:"no-store", headers:{ "apikey":SUPABASE_ANON_KEY, "Authorization":`Bearer ${SUPABASE_ANON_KEY}` } });
  ...
  async function syncFromCloud() {
    ...
    if (cloudData && Array.isArray(cloudData.students)) {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
      cloudStatus = "ok";
      if (!studentsFormVisible && !editingStudentId && !loginModalVisible && activeView !== 'register') render();
  ...
  /* LOGICA DE NEGOCIO */
  function planTotal(student){ if(!student)return 0; if(student.planType==="custom")return Math.max(0,Number(student.customClasses||0)); return Math.max(0,Number(PLAN_CLASSES[student.planType]||student.planType||0)); }
  ...
  function login(user,pass){
    if(user.trim()===ADMIN_USER && pass===ADMIN_PASS){
      localStorage.setItem("acqua.session","1");
      ...
      setToast("Usuario o contrasena incorrectos.");
      const form=document.getElementById("loginForm");
      if(form){ form.style.animation="none"; requestAnimationFrame(()=>{ form.style.animation="shake .35s ease"; }); }
    }
  }
  function logout(){ localStorage.removeItem("acqua.session"); isAuthenticated=false; activeView="checkin"; render(); setToast("Sesion cerrada."); }
  ...
  /* RENDER PRINCIPAL */
  function render(){
    if(!isAuthenticated && activeView!=="checkin" && activeView!=="register") activeView="checkin";
    setActiveTab();
    ...
    if(loginModalVisible)   html+=renderLoginModal();
    // Preservar valores de inputs activos antes de re-renderizar
    const activeId=document.activeElement?.id;
    const inputVals={};
    document.querySelectorAll('#loginForm input,#studentFormPublic input,#studentFormPublic select,#studentFormPublic textarea').forEach(el=>{ if(el.name) inputVals[el.id||el.name]=el.value; });
    app.innerHTML=html;
    // Restaurar valores preservados
    Object.entries(inputVals).forEach(([key,val])=>{
      const el=document.querySelector(`[id="${key}"],[name="${key}"]`);
  ...
  function renderStudentForm(student){
    const data=student||{name:"",category:"Nino",age:"",responsible:"",phone:"",email:"",planType:"4",customClasses:1,status:"active",notes:""};
    ...
    return `<form id="studentForm" class="field-grid">
      ...
      <label class="field">Tipo<select name="category">${so("Nino",data.category)}${so("Adulto",data.category)}${so("Practica libre",data.category)}</select></label>
      <label class="field">Edad<input name="age" type="number" min="0" max="110" value="${escapeHtml(data.age)}"></label>
      ...
      <label class="field">Plan mensual<select name="planType" data-plan-select>
        ${so("1_practica",data.planType,PLAN_LABELS["1_practica"])}${so("4",data.planType,PLAN_LABELS["4"])}${so("8",data.planType,PLAN_LABELS["8"])}${so("8_practicas",data.planType,PLAN_LABELS["8_practicas"])}${so("13",data.planType,PLAN_LABELS["13"])}${so("custom",data.planType,PLAN_LABELS.custom)}</select></label>
      ...
      <label class="field">Estado<select name="status">${so("active","active","Activo")}${so("paused","paused","Pausado")}</select></label>
      <label class="field wide">Notas<textarea name="notes">${escapeHtml(data.notes)}</textarea></label>
    ...
    return `
      ...
      <section class="panel" style="max-width:560px;margin:0 auto">
        ...
        <form id="studentFormPublic" class="field-grid">
          ...
          <label class="field">Edad<input name="age" type="number" min="0" max="110" placeholder="Ej: 8"></label>
          ...
          <label class="field wide">Notas u observaciones<textarea name="notes" placeholder="Alergias, condiciones medicas, horario preferido..."></textarea></label>
...
/* CARNETS QR */
  ...
  function renderQrCard(student){
    ...
    return `<article style="position:relative;width:270px;height:450px;border-radius:18px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.4);break-inside:avoid;page-break-inside:avoid;flex-shrink:0">
      ...
      <!-- Nombre en dos lineas, centrado -->
      <div style="position:absolute;top:67%;left:0;right:0;text-align:center;padding:0 10px;color:#0a2472;font-weight:800;letter-spacing:.04em;line-height:1.3">
        <div style="font-size:1.2rem">${escapeHtml(nombres)}</div>
        ${apellidos?`<div style="font-size:1.2rem">${escapeHtml(apellidos)}</div>`:''}
      </div>
    </article>`;
  }
  ...
  function renderEvents(){
    ...
    return `${renderHeader("Eventos y travesias","Gestiona inscritos, pagos y firmas de exoneracion para actividades fuera de clase.")}<section class="events-layout"><div class="panel"><div class="panel-header"><div><h3>Nuevo evento</h3><p>Crea una competencia, salida o travesia.</p></div></div><form id="eventForm" class="field-grid"><label class="field wide">Nombre del evento<input name="eventName" required placeholder="Travesia San Andres 2026"></label><label class="field">Fecha<input name="eventDate" type="date" required></label><label class="field">Lugar<input name="eventPlace" required placeholder="Piscina, ciudad o punto de salida"></label><div class="split-actions wide"><button class="btn accent" type="submit">${icon("calendar")}Crear evento</button></div></form></div><div class="panel"><div class="panel-header"><div><h3>Agenda activa</h3><p>${state.events.length} evento${state.events.length===1?"":"s"}.</p></div></div>${ordered.length?`<div class="event-list">${ordered.map(renderEventCard).join("")}</div>`:'<div class="empty">No hay eventos todavia.</div>'}</div></section>`;
  }
  ...
  function renderAttendanceTable(entries){
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Alumno</th><th>Origen</th><th>Tipo</th></tr></thead><tbody>${entries.map(entry=>{ const student=getStudent(entry.studentId); return `<tr><td>${escapeHtml(formatDateTime(entry.at))}</td><td><div class="student-name"><strong>${escapeHtml(student?.name||"Alumno no encontrado")}</strong><small>${escapeHtml(student?.code||entry.studentId)}</small></div></td><td>${(entry.source==="qr"||entry.source==="qr-url")?"QR":"Manual"}</td><td>${entry.extra?'<span class="badge warn">Extra</span>':'<span class="badge ok">Plan</span>'}</td></tr>`; }).join("")}</tbody></table></div>`;
  }
  ...
  function renderLoginModal(){
    return `<div class="modal-backdrop" role="dialog" aria-modal="true" id="loginModal" style="z-index:10000"><div class="modal-panel" style="max-width:380px"><div class="modal-header"><div><img class="modal-logo" src="assets/logo-acqua-blanco.png" alt="ACQUA NATACION"><span class="eyebrow">Area administrativa</span><h3>Acceso al control</h3></div><button class="btn icon" type="button" data-action="close-login" style="color:rgba(255,255,255,.7)">${icon("x")}</button></div><form id="loginForm" style="padding:24px;display:grid;gap:12px"><label class="field">Usuario<input name="username" type="text" autocomplete="username" placeholder="acqua" autofocus></label><label class="field">Contrasena<input name="password" type="password" autocomplete="current-password" placeholder="..."></label><button class="btn primary" type="submit" style="width:100%;justify-content:center;margin-top:4px;min-height:46px">${icon("lock")} Ingresar</button></form></div></div><style>@keyframes shake{0%,100%{transform:translateX(0)}20%{transform:translateX(-8px)}40%{transform:translateX(8px)}60%{transform:translateX(-5px)}80%{transform:translateX(5px)}}</style>`;
  }
  ...
  function saveStudentPublic(form){
    ...
    const student={ id:uid("student"), code, name, category:String(fd.get("category")||"Nino"), age:String(fd.get("age")||"").trim(), responsible:String(fd.get("responsible")||"").trim(), phone, email:String(fd.get("email")||"").trim(), planType, customClasses:planType==="custom"?customClasses:"", status:"active", notes:String(fd.get("notes")||"").trim(), monthKey:state.currentMonth, classesUsed:0, createdAt:now, updatedAt:now };
    state.students.push(student);
    saveState();
    setToast("Alumno registrado: "+name+" (Codigo: "+code+")");
    form.reset();
    // Reset custom classes visibility
  ...
  function saveStudent(form){
    ...
    const student={ ...(existing||{}), id:existing?.id||uid("student"), code:existing?.code||nextStudentCode(), name, category:String(fd.get("category")||"Nino"), age:String(fd.get("age")||"").trim(), responsible:String(fd.get("responsible")||"").trim(), phone:String(fd.get("phone")||"").trim(), email:String(fd.get("email")||"").trim(), planType, customClasses:planType==="custom"?customClasses:"", status:String(fd.get("status")||"active"), notes:String(fd.get("notes")||"").trim(), monthKey:existing?.monthKey||state.currentMonth, classesUsed:Number(existing?.classesUsed||0), createdAt:existing?.createdAt||now, updatedAt:now };
  ...
  function renewStudent(studentId,silent=false,newPlanType=null){
    ...
    student.monthKey=state.currentMonth; student.classesUsed=0;
  ...
  function notifyWhatsApp(student){
    ...
    if(remaining<=0) message+="Tus clases o practicas se han agotado. Te esperamos para renovar tu plan y seguir nadando.";
    else if(remaining===1) message+="Te queda 1 clase o practica disponible. Te recomendamos renovar pronto.";
    else message+=`Tienes ${remaining} clases o practicas disponibles.`;
  ...
  async function startScanner(){
    ...
    if(!hasBarcodeDetector&&!hasJsQr){ setToast("Este navegador no soporta lectura QR. Usa Chrome, Edge o Safari actualizado."); return; }
    try {
      if(hasBarcodeDetector){
        const formats=await BarcodeDetector.getSupportedFormats();
        useJsQr=!formats.includes("qr_code");
        if(!useJsQr) scanner.detector=new BarcodeDetector({formats:["qr_code"]});
        else useJsQr=true;
      } else { useJsQr=true; }
      ...
      setToast("Escaner activo - apunta al codigo QR del carnet.");
    } catch(error){
      console.error(error);
      if(error.name==="NotAllowedError") setToast("Permiso de camara denegado. Habilítalo en ajustes del navegador.");
      else setToast("No pude abrir la camara. Verifica los permisos del navegador.");
    }
  }
  ...
  function exportJson(){
    ...
    try {
      ...
      const studentRows=state.students.map(s=>({ "Codigo":s.code||"", "Nombre":s.name||"", "Categoria":s.category||"", "Plan":PLAN_LABELS[s.planType]||s.planType||"", "Clases totales":planTotal(s), "Clases usadas":Number(s.classesUsed||0), "Clases restantes":remainingClasses(s), "Estado":s.status==="active"?"Activo":"Inactivo", "Acudiente":s.responsible||"", "Telefono":s.phone||"", "Email":s.email||"", "Notas":s.notes||"", "Mes renovacion":s.monthKey||"" }));
  ...
  document.addEventListener("click",event=>{
    ...
    if(viewButton){
      if(viewButton.dataset.admin&&!isAuthenticated){ loginModalVisible=true; render(); return; }
    ...
    switch(action.dataset.action){
      case "open-login":  loginModalVisible=true; render(); window.requestAnimationFrame(()=>document.querySelector("#loginForm [name='username']")?.focus()); break;
      case "close-login": loginModalVisible=false; render(); break;
  ...
  document.addEventListener("submit",event=>{
    ...
    if(form.id==="loginForm"){ const fd=new FormData(form); login(String(fd.get("username")||""),String(fd.get("password")||"")); return; }
    if(form.id==="studentForm" || form.id==="studentFormPublic") { saveStudentPublic(form); return; }
  ...
  updateSyncDot();
  syncFromCloud();
  document.addEventListener("visibilitychange",()=>{ if(document.visibilityState==="visible"&&CLOUD_ENABLED)syncFromCloud(); });
  if(CLOUD_ENABLED) setInterval(syncFromCloud,30_000);
