...
/* CARNETS QR */
  ...
  function saveStudent(form){
    ...
    const student={ ...(existing||{}), id:existing?.id||uid("student"), code:existing?.code||nextStudentCode(), name, category:String(fd.get("category")||"Infantil"), birthdate:String(fd.get("birthdate")||"").trim(), documento:String(fd.get("documento")||"").trim(), responsible:String(fd.get("responsible")||"").trim(), phoneAcudiente:String(fd.get("phoneAcudiente")||"").trim(), phone:String(fd.get("phone")||"").trim(), email:String(fd.get("email")||"").trim(),talla:String(fd.get("talla")||"").trim(),peso:String(fd.get("peso")||"").trim(),sangre:String(fd.get("sangre")||"").trim(),eps:String(fd.get("eps")||"").trim(),camiseta:String(fd.get("camiseta")||"").trim(),planType, customClasses:planType==="custom"?customClasses:"", status:String(fd.get("status")||"active"), nivel:String(fd.get("nivel")||""), talla:String(fd.get("talla")||"").trim(),peso:String(fd.get("peso")||"").trim(),sangre:String(fd.get("sangre")||"").trim(),eps:String(fd.get("eps")||"").trim(),camiseta:String(fd.get("camiseta")||"").trim(),notes:String(fd.get("notes")||"").trim(), monthKey:existing?.monthKey||state.currentMonth, classesUsed:Number(existing?.classesUsed||0), planStartedAt:existing?.planStartedAt||now, createdAt:existing?.createdAt||now, updatedAt:now };
