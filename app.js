(() => {
  ...
  async function deleteEventAttendeeRow(eventId, studentId){
    if (!CLOUD_ENABLED || !isAuthenticated) return true;
    try {
      const authH = await getAuthHeader();
      const url = SUPABASE_URL+"/rest/v1/event_attendees?event_id=eq."+encodeURIComponent(eventId)+"&student_id=eq."+encodeURIComponent(studentId);
      const res = await fetch(url, { method:"DELETE", headers:{ "apikey":SUPABASE_ANON_KEY, "Authorization":authH } });
      return res.ok;
    } catch(e){ console.error("deleteEventAttendeeRow:",e); return false; }
  }
  ...
  async function cloudSave() {
    ...
    try {
      ...
      const attendeeRows = state.events.flatMap(e => (e.attendees||[]).map(a => ({
        event_id:e.id, student_id:a.studentId, paid:Boolean(a.paid), signed:Boolean(a.signed),
      ...
      const ok = (await Promise.all([
        ...
        upsertRows("event_attendees", attendeeRows, "event_id,student_id"),
  ...
  async function cloudLoad() {
    ...
    try {
      ...
      attendeeRows.forEach(a => { const e = eventById[a.event_id]; if (e) e.attendees.push({ studentId:a.student_id, paid:Boolean(a.paid), signed:Boolean(a.signed), signature:a.signature||null, addedAt:a.added_at }); });
...
/* CARNETS QR */
  ...
  function removeEventStudent(eventId,studentId){ const event=getEvent(eventId), student=getStudent(studentId); if(!event||!student)return; if(!window.confirm(`Quitar a ${student.name} de "${event.name}"?`))return; event.attendees=(event.attendees||[]).filter(a=>a.studentId!==studentId); touchEvent(eventId); saveState(); setToast("Alumno retirado del evento."); render(); deleteEventAttendeeRow(eventId,studentId); }
  ...
  function exportEventXlsx(eventId){
    ...
    try {
      ...
      const participantRows=attendees.map(a=>{
        ...
        return {
          "Alumno":s.name||"Alumno eliminado",
          "Codigo":s.code||a.studentId,
          "Telefono":s.phone||"",
          "Acudiente":s.responsible||"",
          "Pagado":a.paid?"Si":"No",
      ...
      const wsParticipants=XLSX.utils.json_to_sheet(participantRows.length?participantRows:[{"Alumno":"Aun no hay alumnos inscritos"}]);
      wsParticipants["!cols"]=[26,10,16,20,9,9,20,16,20,18].map(w=>({wch:w}));
  ...
  /* Limpieza de UNA SOLA VEZ: corrige a los alumnos que quedaron con clases de
     mas por el auto-traslado antiguo (ej. Alicia con 16 en vez de 9). Solo se
     ejecuta si nunca se ha corrido en este dispositivo. No vuelve a tocar
     bonusClasses despues de esto, para no borrar sumas manuales futuras. */
  function limpiarBonusHistorico(){
    if(localStorage.getItem("acqua.bonusFixDone")) return;
    let corregidos=0;
    state.students.forEach(s=>{
      if(Number(s.bonusClasses||0)>0){ s.bonusClasses=0; s.updatedAt=new Date().toISOString(); corregidos++; }
    });
    localStorage.setItem("acqua.bonusFixDone","1");
    if(corregidos>0){ saveState(); setToast("Se corrigieron "+corregidos+" alumno(s) con clases de mas por el error de meses anteriores."); render(); }
  }
  ...
  (async function iniciar(){
    ...
    render();
    updateSyncDot();
    if(isAuthenticated){ await syncFromCloud(); limpiarBonusHistorico(); }
