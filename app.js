(() => {
  ...
  async function syncFromCloud() {
    ...
    if (cloudData && Array.isArray(cloudData.students)) {
      const cleanStudents = (cloudData.students||[]).map(s => ({
        ...s,
        bonusClasses: 0  // Reset bonus incorrectos — las clases siguen en classesUsed
      }));
      state = { version:1, currentMonth: cloudData.currentMonth||state.currentMonth, settings:{ businessName:DEFAULT_BUSINESS, ...(cloudData.settings||{}) }, students:cleanStudents, attendance:cloudData.attendance||[], events:Array.isArray(cloudData.events)?cloudData.events:[], internalEvents:Array.isArray(cloudData.internalEvents)?cloudData.internalEvents:[] };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
      cloudStatus = "ok";
  ...
  function calcularVencimiento(student){
    // Buscar la primera clase registrada del alumno en su plan actual
    const mesRenovado = student.monthKey;
    const asistencias = state.attendance
      .filter(a => a.studentId === student.id)
      .sort((a,b) => new Date(a.at) - new Date(b.at));

    // Primera asistencia desde que se renovó el plan
    let primeraClase = null;
    if(student.planStartedAt){
      primeraClase = new Date(student.planStartedAt);
    } else if(asistencias.length > 0){
      primeraClase = new Date(asistencias[0].at);
    }
    ...
    const diasPlan = {
      "1_practica": 30, "1_grupal": 30, "1_matronat": 30,
      "4": 35, "9": 35, "8_practicas": 30,
      "13": 35, "matronat_4": 35, "matronat_8": 35,
      "anual": 365, "obsequio_1": 30, "obsequio_2": 30, "obsequio_3": 30,
      "custom": 35
    };
    const dias = diasPlan[student.planType] || 35;
  ...
  /* LOGICA DE NEGOCIO */
  function planTotal(student){ if(!student)return 0; if(student.planType==="custom")return Math.max(0,Number(student.customClasses||0)); return Math.max(0,Number(PLAN_CLASSES[student.planType]||0)); }
  ...
  function getAlertStudents(){
    // Alumnos con plan que vence en los proximos 7 dias o ya vencio
    const hoy = new Date();
...
/* CARNETS QR */
  ...
  function renderRenewalModal(){
    ...
    return `<div class="modal-backdrop" role="dialog" aria-modal="true" style="z-index:9998"><div class="modal-panel" style="max-width:460px"><div class="modal-header"><div><span class="eyebrow">Renovacion mensual</span><h3>${escapeHtml(student.name)}</h3></div><button class="btn icon" type="button" data-action="close-renewal-modal" style="color:rgba(255,255,255,.7)">${icon("x")}</button></div><div style="padding:20px">${renewModalChangePlan?`<p style="margin:0 0 14px;color:var(--muted);font-size:.88rem">Selecciona el nuevo plan:</p><div style="display:grid;gap:8px">${planBtns}</div><div class="split-actions"><button class="btn ghost" data-action="renewal-back-options">Volver</button></div>`:`<p style="margin:0 0 18px;color:var(--muted);font-size:.88rem">Plan actual: <strong>${escapeHtml(PLAN_LABELS[student.planType]||"Personalizado")}</strong> - Mes: <strong>${escapeHtml(monthLabel(state.currentMonth))}</strong></p><div style="display:grid;gap:10px"><button class="renewal-opt-btn success" data-action="confirm-renewal-same"><span class="renewal-opt-icon">+</span><span><strong>Mismo plan</strong><small>${escapeHtml(PLAN_LABELS[student.planType]||"Personalizado")} - sin cambios</small></span></button><button class="renewal-opt-btn" data-action="show-renewal-change-plan"><span class="renewal-opt-icon">~</span><span><strong>Plan diferente</strong><small>Elige otro plan para este mes</small></span></button><button class="renewal-opt-btn accent" data-action="confirm-renewal-individual"><span class="renewal-opt-icon">1</span><span><strong>Clase individual</strong><small>Solo 1 clase - practica suelta</small></span></button>
                <button class="renewal-opt-btn" style="border-color:#34c759;background:rgba(52,199,89,.06)" data-action="add-one-class"><span class="renewal-opt-icon" style="color:#34c759">+1</span><span><strong>Sumar 1 clase al plan</strong><small>Se suma a las clases disponibles actuales</small></span></button>
  ...
  function showCheckinScreen(student,source="qr"){
    ...
    // Registrar fecha de inicio del plan en la primera clase si no existe
    if(!student.planStartedAt){
  ...
  function notifyVencimiento(student){
    ...
    // Limpiar el numero pero preservar el + inicial si existe
    const hasPlus = rawPhone.startsWith("+");
    const digits = rawPhone.replace(/\D/g,"");
    if(!digits){ setToast("El telefono del alumno no parece valido."); return; }
    // Si ya tiene codigo de pais (mas de 10 digitos o empieza con +), usar tal cual
    // Si es colombiano de 10 digitos empezando en 3, agregar 57
    let phone;
    ...
    if(diasRestantes!==null && diasRestantes<=0){
      msg += "Esperamos que estes muy bien. Queremos recordarte que tu plan esta proxximo a su fin. Comunicate con nosotros para renovarlo y seguir nadando.";
  ...
  function exportJson(){
    ...
    // Guardar timestamp del ultimo respaldo
    localStorage.setItem("acqua.lastBackup", new Date().toLocaleString("es-CO"));
    try {
      ...
      const studentRows=state.students.map(s=>{
        ...
        return {
          ...
          "Inicio del plan":fmtDate(s.planStartedAt), "Fecha vencimiento":(()=>{ try{ const v=calcularVencimiento(s); return v?fmtDate(v.toISOString()):""; }catch(e){return "";} })(),
      ...
      wsStudents["!cols"]=[8,26,14,12,14,8,7,8,10,16,10,20,10,10,10,12,10,14,20,14,24,24,12,14,7,12,16].map(w=>({wch:w}));
  ...
  document.addEventListener("click",event=>{
    ...
    switch(action.dataset.action){
      ...
      case "add-one-class": { const stAdd=getStudent(renewModalStudentId); if(stAdd){ stAdd.bonusClasses=(Number(stAdd.bonusClasses||0)+1); stAdd.updatedAt=new Date().toISOString(); saveState(); setToast("1 clase extra sumada a "+stAdd.name+". Total disponibles: "+(remainingClasses(stAdd))); } renewModalStudentId=null; renewModalChangePlan=false; render(); break; }
  ...
  function autoCheckMonth(){
    ...
    // Solo actualizar el mes operativo para reportes
    // Las clases de cada alumno dependen de su fecha de vencimiento, no del mes
  ...
  /* PING ANTI-PAUSA — evita que Supabase pause el proyecto por inactividad
     Hace una consulta mínima cada 4 días (345600000 ms)                    */
  async function pingSupabase(){
    if(!CLOUD_ENABLED) return;
    try {
      await fetch(SUPABASE_URL+"/rest/v1/app_state?id=eq.ping-keepalive&select=id",{
        cache:"no-store",
        headers:{ apikey:SUPABASE_ANON_KEY, Authorization:"Bearer "+SUPABASE_ANON_KEY }
      });
      console.log("Supabase ping OK -",new Date().toLocaleString("es-CO"));
    } catch(e){ console.warn("Supabase ping fallido:",e.message); }
  }
  ...
  (async function iniciar(){
    ...
    document.addEventListener("visibilitychange",()=>{
      if(document.visibilityState==="visible"&&CLOUD_ENABLED&&isAuthenticated){ syncFromCloud(); autoCheckMonth(); }
    });
    if(CLOUD_ENABLED) setInterval(()=>{ if(isAuthenticated) syncFromCloud(); },30_000);
    // Ping anti-pausa: ejecutar inmediatamente y luego cada 4 dias
    pingSupabase();
    setInterval(pingSupabase, 345_600_000);
    // Verificar cambio de mes cada hora
    setInterval(()=>{ if(isAuthenticated) autoCheckMonth(); },3_600_000);
    // También verificar al volver la pestaña activa
    document.addEventListener('visibilitychange',()=>{ if(document.visibilityState==='visible'&&isAuthenticated) autoCheckMonth(); });
  })();
