(() => {
  ...
  function planEndDate(student){
    if(!student.planStartedAt) return null;
    const start=new Date(student.planStartedAt);
    if(isNaN(start.getTime())) return null;
    const dias=diasVigenciaPlan(student.planType);
    return new Date(start.getTime()+dias*24*60*60*1000);
  }
  function renderStudentsTable(students){
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Alumno</th><th>Plan</th><th>Inicio plan</th><th>Vence</th><th>Clases</th><th>Estado</th><th></th></tr></thead><tbody>${students.map(student=>{
      ...
      const endDate=planEndDate(student);
      const vencido=endDate&&endDate.getTime()<Date.now();
      const venceHtml=endDate? `<span style="color:${vencido?'var(--danger)':'var(--muted)'};font-weight:${vencido?700:400}">${new Intl.DateTimeFormat("es-CO",{day:"2-digit",month:"short",year:"numeric"}).format(endDate)}</span>` : "—";
...
/* CARNETS QR */
  ...
  function exportJson(){
    ...
    try {
      ...
      const fmtDate=iso=>iso?new Intl.DateTimeFormat("es-CO",{dateStyle:"short"}).format(new Date(iso)):"";
      const fmtPesos=v=>v?new Intl.NumberFormat("es-CO",{style:"currency",currency:"COP",maximumFractionDigits:0}).format(Number(v)):"";
      const studentRows=state.students.map(s=>{
        ...
        return {
          ...
          "Vence el":(()=>{ const end=planEndDate(s); return end?fmtDate(end):""; })(),
  ...
  document.addEventListener("click",event=>{
    ...
    switch(action.dataset.action){
      ...
      case "send-carnet-whatsapp": {
        ...
        const digC=String(stC.phone||"").replace(/\D/g,"");
        const phoneC=digC.length===10&&digC.startsWith("3")?"57"+digC:digC;
  ...
  function autoCheckMonth(){
    ...
    // Trasladar clases restantes de alumnos renovados en el mes anterior
    let migrados = 0;
    state.students.forEach(student => {
      if(student.status !== "active") return;
      if(student.monthKey !== oldMonth) return; // no renovado el mes anterior
      const remaining = remainingClasses(student);
      if(remaining > 0){
        // Sumar las clases restantes como bonus en el nuevo mes
        student.bonusClasses = (Number(student.bonusClasses||0)) + remaining;
        migrados++;
      }
      // Marcar que necesita renovación en el nuevo mes
      // (monthKey queda en oldMonth para que aparezca como "Por renovar")
    });

    saveState();
    if(migrados > 0){
      setToast("Mes actualizado a "+monthLabel(realMonth)+". "+migrados+" alumno(s) con clases trasladadas.");
    } else {
      setToast("Mes actualizado automaticamente a "+monthLabel(realMonth)+".");
    }
    render();
  }
