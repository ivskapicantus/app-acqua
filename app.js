(() => {
  ...
  async function cloudLoad() {
    ...
    try {
      ...
      attendeeRows.forEach(a => { const e = eventById[a.event_id]; if (e) e.attendees.push({ id:a.id, studentId:a.student_id||null, guestName:a.guest_name||undefined, guestAge:a.guest_age||undefined, guestPhone:a.guest_phone||undefined, paid:Boolean(a.paid), signed:Boolean(a.signed), signature:a.signature||null, addedAt:a.added_at }); });
...
/* CARNETS QR */
  ...
  function enrollEventGuest(eventId,name,age,phone){
    ...
    event.attendees.push({ id:uid("eatt"), studentId:null, guestName:nm, guestAge:String(age||"").trim(), guestPhone:String(phone||"").trim(), paid:false, signed:false, signature:null, addedAt:new Date().toISOString() });
    event.updatedAt=new Date().toISOString();
    saveState(); setToast(`${nm} inscrito(a) como particular en el evento.`); render();
  }
  function toggleEventPaid(eventId,attendeeId){ const attendee=findEventAttendeeById(eventId,attendeeId); if(!attendee)return; attendee.paid=!attendee.paid; touchEvent(eventId); saveState(); render(); }
